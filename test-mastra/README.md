...
A reference AI agent template that ships with a working Slack triage use case. Built with [Mastra](https://mastra.ai), [Next.js](https://nextjs.org), and [Arcade](https://arcade.dev).
...
You can also manually configure the model in `src/mastra/agents/triage-agent.ts`.
...
│   ├── agents/
│   │   ├── triage-agent.ts         # Agent definition (customization point)
...
1. **Mastra agent** (`src/mastra/agents/triage-agent.ts`) defines the AI agent with a system prompt and tools
2. **MCPClient** (`src/mastra/tools/arcade.ts`) connects to Arcade's MCP Gateway using OAuth authentication — no API keys needed
3. **Chat API route** (`app/api/chat/route.ts`) streams agent responses using `handleChatStream` from `@mastra/ai-sdk`
