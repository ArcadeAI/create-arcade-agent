...
const callbackUrl =
  ensureScheme(process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000") +
  "/api/auth/arcade/callback";
...
let pendingAuthUrlTimestamp = 0;

function setPendingAuthUrl(url: string) {
  pendingAuthUrl = url;
  pendingAuthUrlTimestamp = Date.now();
}

export function getPendingAuthUrl(): string | null {
  if (!pendingAuthUrl) return null;
  if (Date.now() - pendingAuthUrlTimestamp > PENDING_AUTH_TTL_MS) {
    clearPendingAuthUrl();
    return null;
  }
