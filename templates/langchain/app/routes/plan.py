...
def _build_plan_prompt():
    return (
        ...
        "services (e.g. Slack, Google Calendar, Linear, GitHub, Gmail). Your job is to thoroughly scan all "
        "available sources, read recent items AND currently assigned work, and classify each one.\n\n"
        "WORKFLOW:\n"
...
async def plan(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    async def stream():
        ...
        try:
            ...
            TRIAGE_TOOLS = {
                # GitHub
                "Github_ListNotifications", "Github_GetNotificationSummary",
                "Github_ListPullRequests", "Github_GetPullRequest",
                "Github_GetUserOpenItems", "Github_GetUserRecentActivity",
                "Github_GetReviewWorkload", "Github_GetIssue", "Github_WhoAmI",
                # Gmail
                "Gmail_ListEmails", "Gmail_ListThreads", "Gmail_GetThread",
                "Gmail_SearchThreads", "Gmail_WhoAmI",
                # Google Calendar
                "GoogleCalendar_ListEvents", "GoogleCalendar_ListCalendars",
                "GoogleCalendar_WhoAmI",
                # Linear
                "Linear_GetNotifications", "Linear_GetRecentActivity",
                "Linear_ListIssues", "Linear_GetIssue",
                "Linear_ListProjects", "Linear_GetProject", "Linear_WhoAmI",
                # Slack
                "Slack_ListConversations", "Slack_GetMessages",
                "Slack_GetConversationMetadata", "Slack_WhoAmI",
            }
            ...
            tools = [_wrap_tool(t) for t in all_tools if t.name in TRIAGE_TOOLS and not t.name.endswith("_WhoAmI")]

            tool_names = [t.name for t in tools]
            sources = list(set(_map_tool_to_source(n) for n in tool_names if n))
            print(f"[plan] {len(tool_names)} triage tools (of {len(all_tools)} total) from sources: {', '.join(sources)}")
            ...
            user_message = (
                ...
                f"{tool_inventory}\n\n"
                f"In your FIRST step, call ALL non-WhoAmI tools in parallel — one call per tool. Do NOT skip any source.\n"
                f"For calendar tools, make sure to pass today's date range so you get today's events.\n"
                f"After getting results, classify every item. If any source returned a list you can drill into, make follow-up calls.\n"
                f"I need a COMPLETE picture: notifications, assigned work, upcoming events, and unread messages."
            )
